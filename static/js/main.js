...
function submitRsvp(apiKey) {
	...
	if (num_guests != "") {
		...
		fetch('https://api.airtable.com/v0/appAvSmZJGheuZgRf/Guests%20by%20Grouping', {
				method: 'PATCH',
				headers: {
                    'credentials': "include",
				...
				body: JSON.stringify({
					"records": [
                        {
                          "id": "recrrbhDxGpE1BVfM",
                          "fields": {
                            "Guest Name(s)": "Rosina Conte & Hugo Villanueva",
                            "Headcount": num_guests
                          }
                        },
                    ]
				})
			...
			.then((jsonResponse) => {
				...
				// document.querySelector("#username-update-button").style.display = "none";

				// if (jsonResponse.does_username_exist) {
				// 	document.querySelector(".auth-password").style.display = "block";
				// 	document.querySelector(".new-user").style.display = "none";
				// } else {
				// 	document.querySelector(".auth-password").style.display = "block";
				// 	document.querySelector(".existing-username").style.display = "none";
				// }
			});
	}
}
